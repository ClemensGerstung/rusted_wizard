...
pub fn draw<'a, B: Backend>(f: &mut Frame<B>, app: &'a ui_app::App) {
    if app.game.is_some() {
        ...
        if game.state == WizardState::Init {
            ...
            let horizontal_constraints = vec![Constraint::Percentage(30),
            ...
            let mut bg_area = Layout::default()
                .direction(Direction::Vertical)
                .constraints(vec![Constraint::Percentage(40), Constraint::Length(game.player_count.clone() as u16), Constraint::Percentage(50)].as_ref())
            ...
            for player_index in 0..game.player_count {
                ...
                let paragraph = Paragraph::new(name.clone())
                    .style(style);
                ...
                let index_paragraph = Paragraph::new(index_string)
                    .style(style);
